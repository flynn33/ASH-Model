...
TEXT_EXTENSIONS = {".md", ".txt", ".json", ".yaml", ".yml", ".py", ".tex", ".bib"}
EXCLUDED_DIRS = {
    ".git",
    ".vs",
    "__pycache__",
...
REQUIRED_AGENT_IDS = ("technical", "support", "research_theory")
...
def validate_config(config: dict[str, Any], root: Path) -> list[str]:
    ...
    if tuple(sorted(seen_ids)) != tuple(sorted(REQUIRED_AGENT_IDS)):
        errors.append(
            "discussion_agents.json must define exactly the technical, support, and research_theory agents."
        )
...
def post_discussion_comment(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ash-discussion-agents",
        },
        method="POST",
    )
