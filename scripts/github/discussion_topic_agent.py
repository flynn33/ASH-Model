...
def validate_config(config: dict[str, Any], root: Path) -> list[str]:
    ...
    if family_order != ["technical", "support", "research_theory"]:
        errors.append(
            "discussion_topic_generator.json family_order must be ['technical', 'support', 'research_theory']."
        )

    if config.get("seed_marker_prefix") != "ash-discussion-seed":
        errors.append(
            "discussion_topic_generator.json must preserve the ash-discussion-seed marker prefix."
        )
    ...
    if not isinstance(category_preferences, dict):
        ...
        for family in ("technical", "support", "research_theory"):
...
def graphql_request(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ash-discussion-topic-agent",
        },
        method="POST",
    )
...
def build_discussion_body(candidate: dict[str, Any], marker_prefix: str) -> str:
    ...
    prompt_lines = {
        ...
        "research_theory": [
            "- Which theoretical assumptions or mathematical claims should stay fixed here?",
            "- What linked manuscript, wiki, or simulation surfaces should stay consistent with this discussion?",
        ],
    }
